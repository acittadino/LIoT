...
public abstract class BrokerServiceImpl implements Runnable, BrokerService {
	...
	public void registerToTopic(String topic,
		...
		if (!toNotify.get(topic).contains(brokerServiceReceiver))
	...
	public void unregisterToTopic(String topic, String brokerMessageListenerId) {
		if (toNotify.containsKey(topic) && toNotify.get(topic) != null) {
			Iterator<BrokerMessageListenerService> it = toNotify.get(topic)
					.iterator();
			while (it.hasNext()) {
				BrokerMessageListenerService bmlService = it.next();
				if (bmlService.getId().equals(brokerMessageListenerId))
					toNotify.get(topic).remove(bmlService);
			}
