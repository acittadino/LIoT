	...
	%>
		...
		<aui:col width="<%= 20 %>">
			<div class="control-panel-home-actions-category">
				<!-- Sysdata-Hook: Inserimento bottoni veloci -->
				<%String categoryIOT = "IOT"; %>
				<c:choose>
					...
					<c:when test="<%= category.equals(categoryIOT) %>">
						<c:if test="<%= PortalPermissionUtil.contains(permissionChecker, ActionKeys.ADD_USER) %>">
							<liferay-portlet:renderURL portletName="broker_WAR_LIoTBrokerportlet" var="addBrokerURL">
								
							</liferay-portlet:renderURL>

							<p>
								<liferay-ui:message key="Do you want to create a Broker?" />
							</p>

							<aui:button href="<%= addBrokerURL %>" id="controlPanelHomeActionAddBroker" primary="<%= true %>" value="Add Broker" />
						</c:if>
					</c:when>
